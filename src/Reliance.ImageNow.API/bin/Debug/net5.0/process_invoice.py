...
CLUSTER_CENTERS = 'clusterCenter1411NewData.csv'
IMAGE_LINK = 'url'
...
XGB_CLASSIFIER = 'xgboost_model1411NewData.json'
FEATURES_SCALER = 'finalScalerModel3010Final.pkl'
CLUSTER_COLUMNS =['cluster_1','cluster_0']
VENDOR_LABELS = 'vendor_labels_new.csv'
VENDOR_COLUMN = 'Vendor'
VENDOR_CSV='vendor_labels411.csv'
gl_acc_csv = pd.read_csv('gl-accounts-lookup.csv')
subacc_csv = pd.read_csv('costcenter-profitcenter-lookup.csv')
...
def get_prediction(extracted_tuple,xml_file_path):
    cats = ast.literal_eval(read_txt_file('cat_columns.txt'))
    conts = ast.literal_eval(read_txt_file('cont_columns.txt'))
    ...
    output_dict = {"predicted_class":pred,'predicted_probability':probability}
    enclosing_json = {"ml_output":output_dict   , "invoice":claude_output}
    
    return enclosing_json
