...
def clean_values(sample):
    ...
    for key in main_keys:
        sample[key] = 0 if sample[key] == 'Not available' else sample[key]
    return sample
...
def get_validation_set(df, valid_size,y_label,split_denominator=2):
# Calculate the number of samples for the validation set
    valid_df_len = math.ceil(valid_size * len(df))

    # Ensure the validation set length is even for a 50/50 split
    if valid_df_len % 2 != 0:
        valid_df_len -= 1

    # Separate the dataframe by the binary dependent variable
    # assuming that the convention is that the minority class comes first
    class_0 = df[df[y_label] == 0]
    class_1 = df[df[y_label] == 1]

    # Calculate the number of samples needed from each class
    valid_class_len = valid_df_len // split_denominator

    # Sample from each class
    valid_class_0 = class_0.sample(n=valid_class_len)
    valid_class_1 = class_1.sample(n=len(df)-valid_class_len)

    # Combine the samples to form the validation set
    valid_df = pd.concat([valid_class_0, valid_class_1])

    # Create the training set by dropping the validation set samples
